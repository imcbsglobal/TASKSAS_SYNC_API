...
class UploadAccMasterAPI(APIView):
    def post(self, request):
        ...
        # 🔑 IMPORTANT FIX
        # If body is empty → treat as empty list
        data = request.data if isinstance(request.data, list) else []

        try:
            # 🔥 ALWAYS DELETE OLD DATA FIRST
            AccMaster.objects.filter(client_id=client_id).delete()
            logger.info(f"Cleared acc_master for client {client_id}")

            # ✅ EMPTY DATA = CLEAR ONLY (SYNC EXPECTS THIS)
            if len(data) == 0:
                return Response(
                    {"message": "Old acc_master data cleared. No new data received."},
                    status=200
                )
            ...
            return Response(
                {"message": f"{len(data)} acc_master records synced"},
                status=201
            )

        except Exception as e:
            logger.error(
                f"UploadAccMasterAPI failed: {e}\n{traceback.format_exc()}"
            )
            return Response({"error": str(e)}, status=500)
